...
function validateDailySlip(data, result) {
  ...
  const fat = numberValue(data.fat ?? data.fat_percentage);
  const snf = numberValue(data.snf ?? data.snf_percentage);
...
function validateSettlementSlip(data, result) {
  ...
  const income = numberValue(data.total_income ?? data.total_milk_income);
  const feed =
    numberValue(data.feed_deduction ?? data.cattle_feed_deduction) ||
    numberValue(data.total_deductions) ||
    0;
  const other = numberValue(data.other_deduction ?? data.other_deductions) || 0;
  const net = numberValue(data.net_amount ?? data.net_payable);
