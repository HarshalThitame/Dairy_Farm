...
function addConfidenceRules(result, confidence) {
  if (confidence < 0.6) {
    result.errors.push("AI confidence 60% पेक्षा कमी आहे. थेट save करू नका; हाताने नोंद तपासा.");
  ...
  if (confidence < 0.85) {
    result.warnings.push("AI confidence 85% पेक्षा कमी आहे. आवश्यक आकडे हाताने दुरुस्त करा.");
    result.requires_manual_correction = true;
    return;
  }

  if (confidence < 0.95) {
    result.warnings.push("AI confidence 95% पेक्षा कमी आहे. कृपया सर्व आर्थिक आकडे तपासा.");
  }
}
...
function validateDailySlip(data, result) {
  ...
  if (liters !== null && rate !== null && amount !== null) {
    ...
    if (Math.abs(calculated - amount) > Math.max(0.5, amount * 0.005)) {
...
function validateSettlementSlip(data, result) {
  ...
  const roundFig = numberValue(data.round_figure ?? data.round_fig);
  if (roundFig !== null && net !== null && Math.abs(roundFig - net) > 0.5 && Math.abs(roundFig - net) > Math.abs(net * 0.01)) {
    result.warnings.push(`गोल आकडा (₹${roundFig}) आणि शिल्लक रक्कम (₹${net}) मध्ये फरक आहे. कृपया तपासा.`);
  }
