...
export function sanitizeGoalSettings(body = {}, current = {}) {
  ...
  for (const field of numberFields) {
    if (safeBody[field] !== undefined) {
      ...
      merged[field] = Number.isFinite(value) && value >= 0 ? round(value, field.includes("milk") ? 2 : 1) : merged[field];
    }
  }

  if (safeBody.enabled !== undefined) {
    merged.enabled = Boolean(safeBody.enabled);
  }
...
export function validateGoalInput(body = {}) {
  const safeBody = body && typeof body === "object" && !Array.isArray(body) ? body : {};
  const labels = {
    daily_milk_goal: "दैनिक दूध लक्ष्य",
    weekly_milk_goal: "साप्ताहिक दूध लक्ष्य",
    monthly_milk_goal: "मासिक दूध लक्ष्य",
    fat_goal: "फॅट लक्ष्य",
    snf_goal: "SNF लक्ष्य"
  };

  for (const field of Object.keys(labels)) {
    if (safeBody[field] === undefined || safeBody[field] === "" || safeBody[field] === null) {
      continue;
    }

    const value = Number(safeBody[field]);
    if (!Number.isFinite(value)) {
      return `${labels[field]} मध्ये योग्य आकडा भरा.`;
    }
    if (value < 0) {
      return `${labels[field]} शून्य किंवा त्यापेक्षा जास्त असावे.`;
    }
  }
...
export function normalizeGoalSettings(row = {}) {
...
export async function calculateGoalHistory(supabase, farmId, goals) {
  ...
  return Promise.all([
    progressForPeriod(supabase, farmId, goals, "daily_milk", iso(yesterday), iso(yesterday)),
    progressForPeriod(supabase, farmId, goals, "weekly_milk", iso(lastWeekStart), iso(lastWeekEnd)),
    progressForPeriod(supabase, farmId, goals, "monthly_milk", iso(lastMonth), iso(endOfMonth(lastMonth))),
    progressForPeriod(supabase, farmId, goals, "fat", iso(lastMonth), iso(endOfMonth(lastMonth))),
    progressForPeriod(supabase, farmId, goals, "snf", iso(lastMonth), iso(endOfMonth(lastMonth))),
    progressForPeriod(supabase, farmId, goals, "monthly_milk", iso(twoMonthsAgo), iso(endOfMonth(twoMonthsAgo)))
  ]);
}
...
export function buildGoalRecommendation(progress = []) {
  ...
  if (monthly?.target > 0) {
    ...
    return toMarathiNumerals(`तुम्ही मासिक लक्ष्याच्या ${monthly.percentage}% पूर्ण केले आहे. अजून ${monthly.remaining} लिटर बाकी आहे.`);
  }

  if (daily?.target > 0) {
    return toMarathiNumerals(`आजच्या लक्ष्याच्या ${daily.percentage}% दूध जमा झाले आहे.`);
  }

  if (fat?.target > 0) {
    return toMarathiNumerals(`या महिन्याचा सरासरी फॅट ${fat.currentValue}% आहे. लक्ष्य ${fat.target}% आहे.`);
  }
...
function notificationContent(progress) {
  if (progress.goalType === "daily_milk") {
    return {
      ...
      message: toMarathiNumerals(`आजचे दूध लक्ष्य पूर्ण झाले. ${progress.currentValue} ${progress.unit} दूध जमा झाले.`)
    };
  }
  if (progress.goalType === "monthly_milk") {
    return {
      title: "🎉 मासिक दूध लक्ष्य पूर्ण",
      message: toMarathiNumerals(`या महिन्याचे दूध लक्ष्य पूर्ण झाले. ${progress.currentValue} ${progress.unit} दूध जमा झाले.`)
    };
  }
  return {
    title: `🎉 ${progress.label} लक्ष्य पूर्ण`,
    message: toMarathiNumerals(`${progress.label} लक्ष्य पूर्ण झाले. सध्याचे मूल्य ${progress.currentValue}${progress.unit ? ` ${progress.unit}` : ""}.`)
  };
}
