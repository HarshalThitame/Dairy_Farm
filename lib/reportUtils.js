...
export const expenseCategories = ["चारा", "औषध", "रेतन खर्च", "पशुवैद्यक", "मजुरी", "इतर"];
...
export function getNextVaccinationDate(lastDate, vaccineType) {
  ...
  nextDate.setMonth(nextDate.getMonth() + getIntervalMonths(vaccineType));
...
export function displayFinanceCategory(category) {
  return category === "AI खर्च" ? "रेतन खर्च" : category || "इतर";
}
...
<body>
  ...
  ${
    include("finance")
      ? `<section><h2>हिशोब सारांश</h2><p>उत्पन्न: ${escapeHTML(formatCurrency(finance.totalIncome || 0))}</p><p>मासिक खर्च: ${escapeHTML(formatCurrency(finance.totalExpense || 0))}</p><p>वार्षिक खर्च: ${escapeHTML(formatCurrency(finance.annualExpense || 0))}</p><p>मासिक नफा: ${escapeHTML(formatCurrency(finance.netProfit || 0))}</p></section>`
      : ""
  }
  ${
    include("performance")
