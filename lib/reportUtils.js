...
export function isValidReportMonthYear(month, year) {
  const parsedMonth = Number(month);
  const parsedYear = Number(year);
...
export function normalizeReportMonthYear(value = {}, fallback = getIndiaMonthParts()) {
  const month = Number(value.month);
  const year = Number(value.year);

  if (isValidReportMonthYear(month, year)) {
    return { month, year };
  }
...
export function getReportMonthFromSearchParams(searchParams, fallback = getIndiaMonthParts()) {
  return normalizeReportMonthYear(
    {
      month: searchParams?.get?.("month") ?? fallback.month,
      year: searchParams?.get?.("year") ?? fallback.year
    },
    fallback
  );
}

export function getReportYearFromSearchParams(searchParams, fallbackYear = getIndiaMonthParts().year) {
