...
13. Read the printed total row under the सकाळ column as morning_total_liters and the printed total row under the संध्याकाळ column as evening_total_liters. If those printed totals are not visible, return null for that total.
14. Printed morning_total_liters + printed evening_total_liters is the final total milk for accounting. Daily rows are only detail/checking data and must not override printed session totals.
15. For accounting, slip summary values are final: "एकूण कपात" is the final feed/cattle-feed deduction, and "दूध उत्पन्न/एकूण उत्पन्न" is the final milk income.
...
- खाद्य कपात / पशुखाद्य / कपात / खर्च = feed_deduction
- इतर कपात = other_deduction
- एकूण कपात / एकूण खर्च = total_deductions (and also feed_deduction if no separate feed deduction shown)
...
- On 15-day settlement slips, read सकाळचे एकूण दूध and संध्याकाळचे एकूण दूध from the printed total row under those columns.
${retryInstruction}`;
}
...
function normalizeStructuredSlip(aiJson, ocr = {}) {
  ...
  const rawFeedDeduction = parseNumber(
    aiJson.feed_deduction ??
      aiJson.cattle_feed_deduction ??
      aiJson.deductions?.feed_deduction ??
      aiJson.deductions?.cattle_feed_deduction
  );
  const rawOtherDeduction = parseNumber(
    aiJson.other_deduction ??
      aiJson.other_deductions ??
      aiJson.deductions?.other_deduction ??
      aiJson.deductions?.other_deductions
  );
  const explicitTotalDeductions = parseNumber(aiJson.total_deductions ?? aiJson.deductions?.total_deductions);
  const otherDeduction = rawOtherDeduction ?? 0;
  const feedDeduction =
    rawFeedDeduction ??
    (explicitTotalDeductions !== null ? explicitTotalDeductions : null);
  const totalDeductions =
    feedDeduction !== null || rawOtherDeduction !== null
      ? roundMoney(Number(feedDeduction || 0) + Number(otherDeduction || 0))
      : null;
