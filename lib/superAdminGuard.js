...
const LOCAL_SUPER_ADMIN_JWT_SECRET = "goshala-super-admin-local-secret-change-before-production";

function getSuperAdminJwtSecret() {
  ...
  if (process.env.NODE_ENV === "production") {
    const error = new Error("SUPER_ADMIN_JWT_SECRET or JWT_SECRET is not configured.");
    error.status = 500;
    throw error;
  }

  return LOCAL_SUPER_ADMIN_JWT_SECRET;
}
...
export async function verifySuperAdmin(request) {
  ...
  let decoded;
  const jwtSecret = getSuperAdminJwtSecret();
