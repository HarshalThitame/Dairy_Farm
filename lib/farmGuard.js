...
export function signFarmToken(user, farm) {
  return jwt.sign(
    {
      userId: user.id,
      farmId: user.farm_id,
...
export async function verifyFarmAccess(request, requiredCowId = null) {
  ...
  if (!decoded.farmId || !decoded.userId) {
    throw authError("गोशाळा जोडलेली नाही.", 403);
  }

  const supabase = getSupabaseServerClient();
  const { data: user, error: userError } = await supabase
    .from("users")
    .select("id, farm_id, mobile, email, name, role, is_active, is_farm_owner")
  ...
  if (requiredCowId) {
    ...
    if (cowError || !cow) {
      throw authError("ही गाय तुमच्या गोशाळेतील नाही.", 403);
    }
  }
