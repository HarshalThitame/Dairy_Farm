...
import { getResponseStyleInstruction } from "@/lib/aiAssistantSettings";
...
export const aiAssistantTools = [
  ...
  },
  {
    type: "function",
    name: "getExpenses",
    description: "Get farm expenses in a date range, including direct expenses and dairy feed deductions.",
    parameters: dateRangeParameters,
    strict: true
  },
  {
    type: "function",
    name: "getProfit",
    description: "Get total income, milk revenue, other income, expenses, and net profit in a date range.",
    parameters: dateRangeParameters,
    strict: true
  },
  {
    type: "function",
    name: "getMilkTrend",
    description: "Get date-wise milk collection trend points in a date range.",
    parameters: dateRangeParameters,
    strict: true
  },
  {
    ...
    description: "Get complete milk, income, expense, and profit summary in a date range.",
    parameters: dateRangeParameters,
    strict: true
  },
  {
    type: "function",
    name: "getFarmStatus",
    description: "Get today's milk and current month summary for the farm.",
    parameters: {
      type: "object",
      properties: {},
      required: [],
  ...
  {
    type: "function",
    name: "getAnimalSummary",
    description: "Get cow and calf counts/status summary for the farm.",
    parameters: {
      type: "object",
      properties: {},
      required: [],
...
export function buildAssistantInstructions(options = {}) {
  const dates = getCurrentDateContext();
  const responseStyleInstruction = getResponseStyleInstruction(options.responseStyle || options.response_style);
...
Previous month range: ${dates.previousMonthStart} to ${dates.previousMonthEnd}.
Yesterday: ${dates.yesterday}.

Hard rules:
1. Answer in pure Marathi only.
2. Never invent numbers. Never guess.
3. For dairy analytics, milk, income, expense, profit, trend, farm status, animal counts/status, fat, SNF, morning milk, evening milk, highest/lowest day, or date-range questions, call one of the provided tools.
...
9. When a tool result contains totalIncome and otherIncome, use totalIncome for profit/accounting answers and mention otherIncome only if it is greater than 0.
10. If a tool result contains permissionDenied, politely say that this information is disabled in AI settings and tell the user to enable it from Settings > AI.
...
- "मागील महिना", "मागच्या महिन्यात", "last month" = ${dates.previousMonthStart} to ${dates.previousMonthEnd}
...
Keep answers clear and useful for a rural Marathi dairy farmer.`;
}
