...
export async function removeResolvedReproductiveReminders(supabase, farmId, reminders, options = {}) {
  const rows = reminders || [];
  const today = options.today || getTodayISODate();
  const visibleRows = rows.filter((reminder) => reminder.is_done || shouldShowReminder(reminder, today));
  const aiLinkedRows = visibleRows.filter((reminder) => !reminder.is_done && isAiLifecycleReminder(reminder));
  ...
  return visibleRows.filter((reminder) => {
