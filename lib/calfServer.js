...
export const CALF_SELECT = `
  *,
  mother:cows!calves_mother_cow_id_fkey(id, name, breed, status),
  converted_cow:cows!calves_converted_cow_id_fkey(id, name, breed, status)
`;
...
export function buildCalfPayload(input, farmId) {
  ...
  const payload = {
    ...
    identification_mark: cleanText(input.identification_mark || input.calf_identification_mark),
...
function reminderRowsForCalf(calf) {
  if (!calf.is_raised || calf.gender !== "मादी") {
    return [];
  }
  ...
  return [
    {
      ...
      message: `${name} आता ४० दिवसांची झाली आहे. दूध कमी करण्यास सुरुवात करा.`,
    ...
    },
    {
      ...
      message: `${name} आता ६० दिवसांची झाली आहे. दूध बंद करण्याची वेळ झाली आहे.`,
      related_record_id: calf.id,
      is_done: false
    }
  ];
