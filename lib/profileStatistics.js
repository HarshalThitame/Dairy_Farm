...
async function fetchMilkRecords(supabase, farmId, startDate, endDate) {
  let query = supabase
    .from("milk_records")
    .select("id, date, cow_id, morning_litres, evening_litres, total_litres, total_amount, fat_percentage, morning_fat_percentage, evening_fat_percentage, snf_value, morning_snf_value, evening_snf_value")
    .eq("farm_id", farmId)
    .is("cow_id", null);
...
async function fetchSettlements(supabase, farmId, startDate, endDate) {
  let query = supabase
    ...
    .select("id, period_start, period_end, total_liters, total_milk_income")
    .eq("farm_id", farmId);
...
async function buildDailyPerformance(supabase, farmId, startDate, endDate) {
  ...
  milkRecords.forEach((record) => {
    ...
    const income = numberOrZero(record.total_amount);
...
async function buildOverview(supabase, farmId, userId) {
  ...
  const settlementMilk = settlements.reduce((sum, row) => sum + numberOrZero(row.total_liters), 0);
...
function fontPath() {
  const localPath = path.join(process.cwd(), "public", "fonts", "NotoSansDevanagari-Regular.ttf");
...
export async function buildProfileStatistics(supabase, farmId, userId) {
  ...
  const [trends, bestPerformance] = await Promise.all([
  ...
  return {
    ...
    trends,
    bestPerformance,
    farmHealthScore: buildHealthScore(daily, { ...currentRange, endDate: today }, overview.totalSlips),
