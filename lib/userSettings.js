...
export function normalizeAppearancePreferences(row = {}) {
  ...
  return {
    ...
    language: ["mr", "en"].includes(source.language) ? source.language : DEFAULT_APPEARANCE.language,
...
export async function getSafeAppearancePreferences(supabase, userId, farmId, fallback = DEFAULT_APPEARANCE) {
  try {
    const preferences = await getOrCreateAppearancePreferences(supabase, userId, farmId);
    return normalizeAppearancePreferences(preferences);
  } catch {
