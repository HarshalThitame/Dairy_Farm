...
export async function requestAndRegisterPushSubscription({ requestPermission = false } = {}) {
  ...
  if (!pushNotificationsSupported()) {
    safeSetLocalStorageItem(PERMISSION_KEY, "unsupported");
  ...
  if (permission !== "granted" && requestPermission) {
    ...
    safeSetLocalStorageItem(PERMISSION_KEY, permission);
    if (permission === "granted") {
      safeRemoveLocalStorageItem(DISMISSED_KEY);
  ...
  const storedVapidKey = safeGetLocalStorageItem(VAPID_KEY_STORAGE, "");
  ...
  safeSetLocalStorageItem(VAPID_KEY_STORAGE, vapidPublicKey);
