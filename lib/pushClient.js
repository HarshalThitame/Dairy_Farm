...
async function registerServiceWorker(scriptUrl, scope, timeoutMs = 8000) {
  try {
    ...
    registration.update?.();
    return waitForActiveRegistration(registration, timeoutMs);
  } catch (error) {
...
async function ensureServiceWorkerRegistration() {
  let registration = await waitForServiceWorkerReady(1500);
  if (registration?.active && registration.pushManager) {
    return registration;
  }

  const existingAppRegistration = await getActiveRegistrationForUrl("/");
  if (existingAppRegistration?.pushManager) {
    return existingAppRegistration;
  }

  const appRegistration = await registerServiceWorker("/sw.js", "/", 3500);
  if (appRegistration?.pushManager) {
    return appRegistration;
  }

  const existingPushRegistration = await getActiveRegistrationForUrl("/push-notifications/");
  if (existingPushRegistration?.pushManager) {
    return existingPushRegistration;
  }

  const pushRegistration = await registerServiceWorker("/push-sw.js", "/push-notifications/", 8000);
