...
import { isAhilyanagarDistrict } from "@/lib/maharashtraLocations";
...
function expandDistrictAliases(districts = []) {
  ...
  districts.forEach((district) => {
    ...
    values.add(cleanDistrict);
    if (isAhilyanagarDistrict(cleanDistrict)) {
      values.add("अहिल्यानगर");
      values.add("अहमदनगर");
    }
  });
...
export function getStoredScheduleConfig(source = {}) {
  ...
  if (scheduleType === "custom_cron" && !cronExpression) {
    throw validationError("Custom cron expression आवश्यक आहे.");
  }
...
export function normalizeNotificationPayload(body = {}) {
  ...
  if (!title) {
    throw validationError("Title आवश्यक आहे.");
  }
  if (!message) {
    throw validationError("Message आवश्यक आहे.");
  }
  ...
  if (!channels.some((channel) => ["in_app", "push"].includes(channel))) {
    throw validationError("किमान एक delivery channel आवश्यक आहे.");
  }

  if (!saveAsDraft && scheduleType !== "now" && !scheduledAt) {
    throw validationError("Schedule date आणि time आवश्यक आहे.");
  }

  if (scheduledAt && !isValidDateTime(scheduledAt)) {
    throw validationError("Schedule date आणि time चुकीचा आहे.");
  }

  if (!saveAsDraft && scheduleType !== "now" && scheduledAt && new Date(scheduledAt).getTime() < Date.now() - 60000) {
    throw validationError("Schedule date आणि time मागच्या वेळेत ठेवू शकत नाही.");
  }

  if (expiresAt && !isValidDateTime(expiresAt)) {
    throw validationError("Expiry date चुकीची आहे.");
  }

  if (expiresAt && scheduledAt && new Date(expiresAt).getTime() <= new Date(scheduledAt).getTime()) {
    throw validationError("Expiry date scheduled date नंतर असणे आवश्यक आहे.");
  }
  ...
  if (requirement && !requirement[0].length) {
    throw validationError(requirement[1]);
  }
...
async function resolveTargetFarms(supabase, payload) {
  ...
  const districts = expandDistrictAliases(targetFilter?.districts || []);
...
async function sendPushForNotification(supabase, notification, recipients) {
  ...
  for (const recipient of allowedRecipients) {
    ...
    if (userDelivered) {
      deliveredUsers += 1;
      deliveredUserIds.push(recipient.user_id);
...
export async function sendNotificationNow(supabase, notificationId) {
  ...
  if (notification.status === "cancelled") {
    throw validationError("Cancelled notification पाठवता येणार नाही.");
  }
  if (notification.status === "sent") {
    throw validationError("Notification आधीच पाठवली आहे.");
  }
  if (notification.expires_at && new Date(notification.expires_at).getTime() < Date.now()) {
    throw validationError("Expired notification पाठवता येणार नाही.");
  }
  ...
  if (!lockedNotification) {
    throw validationError("Notification आधीच process होत आहे.");
  }
  ...
  try {
    ...
    if (!resolved.recipients.length) {
      throw validationError("निवडलेल्या audience मध्ये active users सापडले नाहीत.");
    }
    ...
    let pushStats = { attempted: 0, delivered: 0, failed: 0, configured: false, deliveredUserIds: [] };
    if (pushRequested) {
    ...
    const deliveredRecipientIds = new Set([
      ...inAppRows.map((row) => row.user_id).filter(Boolean),
      ...(pushStats.deliveredUserIds || [])
    ]);
    const deliveredCount = deliveredRecipientIds.size;
    const update = {
      status: "sent",
      sent_at: now,
...
export async function refreshNotificationStats(supabase, notificationId) {
  ...
  const delivered = new Set(
    (logs || [])
      .filter((log) => log.delivery_status !== "failed" && log.user_id)
      .map((log) => log.user_id)
  ).size;
