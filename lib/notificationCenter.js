...
export function normalizeNotificationPayload(body = {}) {
  ...
  if (!asBoolean(body.saveAsDraft) && scheduleType !== "now" && !scheduledAt) {
  ...
  return {
    ...
    saveAsDraft: asBoolean(body.saveAsDraft)
  };
}
...
async function sendPushForNotification(supabase, notification, recipients) {
  if (!getWebPushConfigured() || !recipients.length) {
  ...
  const recipientByUser = new Map(recipients.map((recipient) => [recipient.user_id, recipient]));
  ...
  for (const subscription of subscriptions || []) {
    const recipient = recipientByUser.get(subscription.user_id);
    ...
    try {
      ...
      await supabase.from("notification_delivery_logs").upsert({
        ...
        error_message: error.message || "Push delivery failed",
        retry_count: 1
      }, { onConflict: "notification_id,user_id,channel" });
...
export async function sendNotificationNow(supabase, notificationId) {
  ...
  const resolved = await resolveNotificationRecipients(supabase, payload);
  const inAppRows = resolved.recipients.map((recipient) => ({
  ...
  let pushStats = { attempted: 0, delivered: 0, failed: 0 };
  ...
  const update = {
    ...
    delivered_count: inAppRows.length,
    failure_reason: pushStats.failed ? `${pushStats.failed} push deliveries failed.` : null
  };
...
export async function getAdminNotificationAnalytics(supabase) {
  ...
  return {
    totals: {
      totalSent,
      sentToday: rows.filter((row) => String(row.sent_at || row.created_at || "").startsWith(today)).length,
