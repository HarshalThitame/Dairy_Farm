...
export function calculateAIReminders(aiDate, cowName) {
  return [
    ...
    },
    {
      ...
      message: `${cowName} साठी ६० दिवस पूर्ण झाले आहेत. गर्भधारणा तपासणी नोंद करा.`
    },
    {
...
export function shouldShowReminder(reminder, today = getTodayISODate()) {
  if (!reminder || reminder.is_done) {
    return false;
  }

  // "गर्भधारणा तपासणी बाकी" means the check was missed. Showing it before
  // its due date reads like a duplicate of the normal pregnancy-check reminder.
  if (
    reminder.type === MISSED_PREGNANCY_REMINDER_TYPE &&
    String(reminder.reminder_date || "") > String(today || "")
  ) {
    return false;
  }
...
export function getReminderDisplayMessage(reminder, today = getTodayISODate()) {
  if (!reminder) {
    return "";
  }

  if (reminder.type !== MISSED_PREGNANCY_REMINDER_TYPE) {
    return reminder.message || "";
  }

  const cowName = reminder.cows?.name || reminder.cowName || reminder.cow_name || "गाय";
  const isFuture = String(reminder.reminder_date || "") > String(today || "");

  if (isFuture) {
    return `${cowName} साठी गर्भधारणा तपासणीची नोंद बाकी राहू शकते. तारीख आल्यावर तपासा.`;
  }

  return `${cowName} साठी ६० दिवस पूर्ण झाले आहेत. गर्भधारणा तपासणी नोंद करा.`;
}
