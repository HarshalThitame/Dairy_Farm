...
const MILK_RANKS = [
  { code: "beginner_farmer", label: "नवीन दूध उत्पादक शेतकरी", minScore: 0, icon: "🌱" },
  { code: "active_farmer", label: "सक्रिय दूध उत्पादक शेतकरी", minScore: 20, icon: "🌿" },
  { code: "smart_farmer", label: "स्मार्ट दूध उत्पादक शेतकरी", minScore: 35, icon: "🧠" },
  { code: "advanced_farmer", label: "प्रगत दूध उत्पादक शेतकरी", minScore: 50, icon: "🚀" },
  { code: "expert_farmer", label: "तज्ञ दूध उत्पादक शेतकरी", minScore: 65, icon: "🏅" },
  { code: "dairy_champion", label: "दूध उत्पादक चॅम्पियन", minScore: 80, icon: "🏆" },
  { code: "dairy_master", label: "दूध उत्पादक मास्टर", minScore: 92, icon: "👑" }
];
...
function rankByCode(code) {
  return MILK_RANKS.find((rank) => rank.code === code) || null;
}

function rankChangeDirection(oldRank, newRank) {
  if (!oldRank || !newRank || oldRank.code === newRank.code) return "same";
  if (newRank.minScore > oldRank.minScore) return "increase";
  if (newRank.minScore < oldRank.minScore) return "decrease";
  return "same";
}

function isMissingTalukaColumnError(error) {
  const message = `${error?.message || ""} ${error?.details || ""} ${error?.hint || ""}`.toLowerCase();
  return error?.code === "42703" || message.includes("taluka_name");
}
...
export async function getAchievementMetrics(supabase, auth) {
  ...
  const [
    ...
    safeQuery(supabase.from("dairy_slips").select("id, slip_date, session, liters, rate_per_liter, total_amount, fat_percentage, snf_percentage, created_at").eq("farm_id", farmId), []),
    safeQuery(supabase.from("dairy_settlements").select("id, total_liters, total_milk_income, period_start, period_end, morning_total_liters, evening_total_liters, session_totals, ai_raw_data, created_at").eq("farm_id", farmId), []),
  ...
  const milkIncomeSummary = summarizeMilkIncomeForMonth(dairySlips, settlements);
  const settlementMilk = numberValue(milkIncomeSummary.settlementLiters);
  const slipMilk = sumRows(dairySlips, "liters");
  const recordMilk = sumRows(milkRecords, "total_litres");
  const totalMilk = numberValue(milkIncomeSummary.totalLiters) || Math.max(settlementMilk, slipMilk, recordMilk);

  const settlementIncome = numberValue(milkIncomeSummary.settlementIncome);
  const slipIncome = sumRows(dairySlips, "total_amount");
  const recordIncome = sumRows(milkRecords, "total_amount");
  const totalIncome = numberValue(milkIncomeSummary.totalAmount) || Math.max(settlementIncome, slipIncome, recordIncome);

  const dateValues = [
    ...dairySlips.map((row) => row.slip_date),
    ...milkRecords.filter((row) => numberValue(row.total_litres) > 0).map((row) => row.date)
  ];
  const uniqueRecordDates = Array.from(new Set(dateValues.map(isoDateOnly).filter(Boolean)));
  ...
  const recentActivity = uniqueRecordDates.filter((date) => date >= recentCutoff).length;
...
function normalizeAchievement(achievement, progress, unlockedIds) {
  ...
  const currentValue = round(progress?.current_value || 0);
  ...
  return {
    ...
    currentValue: isUnlocked ? Math.max(currentValue, targetValue) : currentValue,
    progressPercent: isUnlocked ? 100 : clamp(progress?.progress_percent || 0),
    remainingValue: isUnlocked ? 0 : Math.max(0, round(progress?.remaining_value || 0)),
...
async function updateLeaderboardRows(supabase, auth, metrics, scoreData, farm) {
  ...
  const { error } = await supabase.from("leaderboards").upsert(rows, { onConflict: "leaderboard_type,period_key,farm_id" });

  if (isMissingTalukaColumnError(error)) {
    const fallbackRows = rows.map(({ taluka_name, ...row }) => row);
    const { error: fallbackError } = await supabase
      .from("leaderboards")
      .upsert(fallbackRows, { onConflict: "leaderboard_type,period_key,farm_id" });
...
export async function evaluateAchievements(supabase, auth, options = {}) {
  ...
  for (const achievement of definitions) {
    ...
    const targetValue = numberValue(achievement.target_value);
    const wasUnlocked = unlockedIds.has(achievement.id);
    const reachedTarget = currentValue >= targetValue;
    const isUnlocked = reachedTarget || wasUnlocked;
    ...
    progressRows.push({
      ...
      remaining_value: isUnlocked ? 0 : Math.max(0, round(targetValue - currentValue)),
  ...
  const oldRank = rankByCode(existingScore?.rank_code);
  const rankDirection = rankChangeDirection(oldRank, currentRank);
  const rankChanged = existingScore?.rank_code && existingScore.rank_code !== currentRank.code
    ? {
        oldRank: oldRank || { code: existingScore.rank_code, label: existingScore.rank_label, minScore: Number(existingScore.dairy_score || 0) },
        newRank: currentRank,
  ...
  const scorePayload = {
    ...
    taluka_name: metrics.farm?.taluka_name || "",
  ...
  const { error: scoreError } = await supabase.from("user_scores").upsert(scorePayload, { onConflict: "farm_id" });

  if (isMissingTalukaColumnError(scoreError)) {
    const { taluka_name, ...fallbackPayload } = scorePayload;
    const { error: fallbackScoreError } = await supabase.from("user_scores").upsert(fallbackPayload, { onConflict: "farm_id" });
    if (fallbackScoreError) throw fallbackScoreError;
  } else if (scoreError) {
    throw scoreError;
  }
  ...
  const notifications = notify
    ? await sendUnlockNotifications(
        ...
        auth,
        newlyUnlocked,
        rankChanged?.direction === "increase" ? rankChanged : null
      )
    : [];
...
export async function getLeaderboard(supabase, auth, type = "farm", scope = "all") {
  ...
  const { data: currentScoreForScope } = await supabase
    .from("user_scores")
    .select("*")
    .eq("farm_id", auth.farmId)
    .maybeSingle();
  ...
  if (scope === "taluka") {
    if (currentScoreForScope?.taluka_name) {
      query = query.eq("taluka_name", currentScoreForScope.taluka_name);
    } else {
      query = query.eq("farm_id", auth.farmId);
    }
  } else if (type === "district" || scope === "district") {
    if (currentScoreForScope?.district_name) {
      query = query.eq("district_name", currentScoreForScope.district_name);
    }
  }
  ...
  if (!currentFarm) {
    ...
    if (currentScore) {
      ...
      if (scope === "taluka") {
        if (currentScore.taluka_name) {
          rankQuery = rankQuery.eq("taluka_name", currentScore.taluka_name);
        } else {
          rankQuery = rankQuery.eq("farm_id", auth.farmId);
        }
      } else if (type === "district" || scope === "district") {
      ...
      currentFarm = {
        ...
        talukaName: currentScore.taluka_name,
  ...
  return {
    type,
    scope,
    scopeLabel: scope === "taluka" ? (currentScoreForScope?.taluka_name || "माझा तालुका") : "सर्व Farms",
    currentTaluka: currentScoreForScope?.taluka_name || "",
    currentDistrict: currentScoreForScope?.district_name || "",
    rows,
    currentFarm
  };
}
