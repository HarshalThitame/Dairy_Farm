...
function deductionTotal(data = {}) {
  ...
  const explicitFeed = data.cattle_feed_deduction ?? data.feed_deduction ?? deductions.feed_deduction;
  const other = numberValue(data.other_deductions ?? deductions.other_deductions);
  const feed = hasPositiveNumber(explicitFeed)
    ? numberValue(explicitFeed)
    : hasNumber(explicitTotal)
...
export function fillMissingDataWithRules(extractedData = {}, analysis = analyzeSlipForPatterns(extractedData), slipType = extractedData.slip_type) {
  ...
  if (slipType === "settlement" && !hasNumber(filledData.net_payable) && hasPositiveNumber(filledData.total_milk_income)) {
    ...
    addWarningIfSourceEstimated("feed_deduction", "net_payable");
