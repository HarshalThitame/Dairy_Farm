...
function deductionTotal(data = {}) {
  ...
  const explicitTotal = data.total_deductions ?? deductions.total_deductions;

  if (hasNumber(explicitTotal)) {
    return numberValue(explicitTotal);
  }

  return (
    numberValue(data.cattle_feed_deduction ?? data.feed_deduction ?? deductions.feed_deduction) +
    numberValue(data.anamat_cut ?? data.anamat_deduction ?? deductions.anamat_cut) +
    numberValue(data.other_deductions ?? deductions.other_deductions)
  );
}
...
export function fillMissingDataWithRules(extractedData = {}, analysis = analyzeSlipForPatterns(extractedData), slipType = extractedData.slip_type) {
  ...
  if (slipType === "settlement" && Array.isArray(filledData.daily_entries)) {
    filledData.daily_entries = filledData.daily_entries.map((entry, index) => {
      if (!entry) {
        return entry;
      }

      const next = { ...entry };
      const liters = numberValue(next.liters);
      const rate = numberValue(next.rate_per_liter ?? next.rate);

      if (liters > 0 && rate > 0 && !hasPositiveNumber(next.amount ?? next.total_amount)) {
        next.amount = roundMoney(liters * rate);
        remember(`daily_entries[${index}].amount`, next.amount, "calculated", {
          confidence: 0.98,
          formula: `liters x rate = ${liters} x ${rate}`
        });
      }

      if (liters > 0 && !hasPositiveNumber(next.fat_percent ?? next.fat_percentage) && hasPositiveNumber(analysis.averages?.fat)) {
        next.fat_percent = Number(analysis.averages.fat);
        remember(`daily_entries[${index}].fat_percent`, next.fat_percent, "average_from_slip", {
          confidence: 0.72,
          note: "या स्लिपमधील स्पष्ट नोंदींची सरासरी"
        });
      }

      if (liters > 0 && !hasPositiveNumber(next.snf_percent ?? next.snf_percentage) && hasPositiveNumber(analysis.averages?.snf)) {
        next.snf_percent = Number(analysis.averages.snf);
        remember(`daily_entries[${index}].snf_percent`, next.snf_percent, "average_from_slip", {
          confidence: 0.72,
          note: "या स्लिपमधील स्पष्ट नोंदींची सरासरी"
        });
      }
  ...
  const hasAnyDailyMilk = Array.isArray(filledData.daily_entries) && filledData.daily_entries.some((entry) => hasPositiveNumber(entry?.liters));
  const canReconstruct =
    slipType === "settlement" &&
    !hasAnyDailyMilk &&
    filledData.period_start &&
    hasPositiveNumber(filledData.total_milk_income) &&
    (hasPositiveNumber(filledData.total_liters) || hasPositiveNumber(analysis.averages?.rate));

  if (canReconstruct) {
    const days = 15;
    const totalIncome = numberValue(filledData.total_milk_income);
    const totalLiters = hasPositiveNumber(filledData.total_liters)
      ? numberValue(filledData.total_liters)
      : roundMoney(totalIncome / numberValue(analysis.averages.rate));
    const rate = hasPositiveNumber(analysis.averages?.rate) ? numberValue(analysis.averages.rate) : roundMoney(totalIncome / totalLiters);
    const avgDailyAmount = roundMoney(totalIncome / days);
    const avgDailyLiters = roundMoney(totalLiters / days);

    filledData.daily_entries = Array.from({ length: days }, (_, index) => ({
      date: addDays(filledData.period_start, index),
      liters:
        index === days - 1
          ? roundMoney(totalLiters - avgDailyLiters * (days - 1))
          : avgDailyLiters,
      fat_percent: hasPositiveNumber(analysis.averages?.fat) ? Number(analysis.averages.fat) : null,
      snf_percent: hasPositiveNumber(analysis.averages?.snf) ? Number(analysis.averages.snf) : null,
      rate_per_liter: rate,
      amount:
        index === days - 1
          ? roundMoney(totalIncome - avgDailyAmount * (days - 1))
          : avgDailyAmount,
      reconstructed: true
    }));

    remember("daily_entries", "१५ दिवसांची अंदाजित पुनर्रचना", "distributed_from_totals", {
      confidence: 0.35,
      note: "एकूण रक्कम/लिटरवरून समान विभागणी",
      warning: "⚠️ पुनर्रचना केली आहे. प्रत्येक दिवस farmer ने तपासणे आवश्यक आहे."
    });
  }
