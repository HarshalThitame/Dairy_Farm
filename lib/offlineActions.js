...
import {
  ...
  cacheSettlements,
  getCachedAIByCow,
  getCachedAIByDateRange,
  getCachedCow,
  getCachedCows,
  getCachedDairySlips,
  getCachedDoneReminders,
  getCachedFinanceByDateRange,
  getCachedHealthByCow,
  getCachedHealthByDateRange,
  getCachedMilkByDate,
  getCachedMilkByCow,
  ...
  markCachedCowRemindersDone,
  markCachedRelatedRemindersDone,
  markCachedSupersededAIRecordsNegative,
...
const allowedReminderActions = new Set([
  "done",
  "skip",
  "snooze",
  "pregnancy-positive",
  ...
  if (isOnline()) {
    try {
      ...
      return {
        data: cached,
        fromCache: true,
        fetchedAt: safeGetLocalStorageItem("goshala-cows-fetched-at", null)
      };
    }
  }
...
export async function fetchCowProfile(cowId) {
  ...
  if (isOnline()) {
    try {
      const responseData = await fetchJson(`/api/cows/${cowId}`);
      const cowProfile = responseData?.cow ? responseData : responseData?.data;
      if (!cowProfile?.cow) {
        throw new Error("गायीची माहिती अपूर्ण आहे.");
      }

      await updateCachedCow(cowProfile.cow, farmId);
      await cacheAIRecords(cowProfile.records?.ai_records || [], farmId);
      await cacheHealthRecords(cowProfile.records?.health_records || [], farmId);
      return { data: cowProfile, fromCache: false };
    } catch {
...
export async function saveAIRecord(aiData) {
  ...
  if (isOnline()) {
    ...
    await markCachedSupersededAIRecordsNegative(aiData.cow_id, aiData.ai_date, farmId);
    await markCachedCowRemindersDone(
      aiData.cow_id,
      [
        PREGNANCY_CHECK_REMINDER_TYPE,
        MISSED_PREGNANCY_REMINDER_TYPE,
...
export async function saveHealthRecord(healthData) {
  ...
  if (isOnline()) {
    ...
    if (!data) {
      throw new Error("आरोग्य नोंद जतन झाली नाही.");
    }

    await cacheHealthRecords([data], farmId);

    if (result.followupReminder) {
      await cacheReminders([result.followupReminder], farmId);
    }
    ...
    return { success: true, data, followupReminder: result.followupReminder || null, offline: false };
  }
...
export async function fetchReminderDetail(reminderId) {
  ...
  if (isOnline()) {
    try {
      ...
      if (String(error?.message || "").includes("आठवण सापडली नाही")) {
        throw error;
      }
      // Fall through to cache.
    }
...
export async function updateReminderAction(reminderId, action = "done", days) {
  ...
  if (!allowedReminderActions.has(action)) {
    throw new Error("आठवणीची action चुकीची आहे.");
  }

  if (action === "snooze") {
    const snoozeDays = Number(days || 1);
    if (!Number.isInteger(snoozeDays) || snoozeDays < 1 || snoozeDays > 30) {
      throw new Error("आठवण १ ते ३० दिवसांपर्यंतच पुढे ढकलता येते.");
    }
  }
  ...
  if (cachedReminder) {
    if (action === "snooze") {
    ...
    } else if (action === "pregnancy-positive" || action === "pregnancy-negative") {
      await updateCachedReminder({
        ...cachedReminder,
        is_done: true,
        skipped: false,
      ...
      await markCachedRelatedRemindersDone(
        cachedReminder.related_record_id,
        action === "pregnancy-negative"
          ? [
              PREGNANCY_CHECK_REMINDER_TYPE,
              MISSED_PREGNANCY_REMINDER_TYPE,
              DRY_OFF_REMINDER_TYPE,
        ...
        { skipped: action === "pregnancy-negative" },
        farmId
      );
      if (cachedReminder.related_record_id) {
        await updateCachedAIRecord({
          id: cachedReminder.related_record_id,
          farm_id: farmId,
          cow_id: cachedReminder.cow_id || null,
          pregnancy_result: action === "pregnancy-negative" ? "negative" : "positive",
          _pending: true
        }, farmId);
      }
      if (cachedReminder.cow_id) {
        const cachedCow = await getCachedCow(cachedReminder.cow_id, farmId);
        if (cachedCow) {
          await updateCachedCow({
            ...cachedCow,
            status: action === "pregnancy-negative" ? "रिकामी" : "गाभण"
          }, farmId);
        }
      }
    }
...
export async function saveFinanceRecord(financeData) {
  ...
  if (isOnline()) {
    ...
    await cacheFinanceRecords([data], farmId);
    return { success: true, data, offline: false };
  }
  ...
  await addLocalFinanceRecord(localFinance, farmId);
  const localId = await queueAction({
    type: "CREATE",
    entity: "finance",
  ...
  return { success: true, data: localFinance, offline: true, localId };
}
...
export async function uploadSlipImage(imageBlob, metadata = {}) {
  ...
  let compressed = null;
  let clientPrepared = true;

  try {
    compressed = await compressImageFileToWebP(imageBlob);
  } catch {
    clientPrepared = false;
    compressed = {
      compressedBlob: imageBlob,
      compressedFile: imageBlob,
      originalSize: imageBlob?.size || 0,
      compressedSize: imageBlob?.size || 0,
      compressionRatio: 0,
      format: imageBlob?.type || "image",
      originalFilename: metadata.originalFilename || imageBlob?.name || "dairy-slip.jpg",
      compressedFilename: metadata.originalFilename || imageBlob?.name || "dairy-slip.jpg",
  ...
  const validation = validateImageSize(compressed.compressedSize, {
    maxSize: clientPrepared ? 25 * 1024 * 1024 : 25 * 1024 * 1024
  });
  ...
  formData.append("clientCompressed", String(clientPrepared && !compressed.needsServerCompression));
  formData.append("clientPrepared", String(clientPrepared && !compressed.needsServerCompression));
...
export async function fetchAccountingSummary(month, year) {
  ...
  const range = getMonthRange(month, year);
  const [healthRecords, aiRecords, financeRecords] = await Promise.all([
    getCachedHealthByDateRange(range.start, range.end, getFarmId()),
    getCachedAIByDateRange(range.start, range.end, getFarmId()),
    getCachedFinanceByDateRange(range.start, range.end, getFarmId())
  ]);
  ...
  const cachedExpenseRows = expensesResult.data.expenses || [];
  const directExpenseRows = cachedExpenseRows.filter((expense) => expense.source !== "dairy_settlements");
  const expenses = summarizeExpenses(directExpenseRows);
  const hasCachedHealthExpenses = directExpenseRows.some((expense) => expense.source === "health_records");
  const hasCachedAIExpenses = directExpenseRows.some((expense) => expense.source === "ai_records");
  const hasCachedFinanceExpenses = directExpenseRows.some((expense) => expense.source === "finance_records");
  const healthExpense = hasCachedHealthExpenses
    ? 0
    : healthRecords.reduce((sum, record) => sum + Number(record.cost || 0), 0);
  const aiExpense = hasCachedAIExpenses
    ? 0
    : aiRecords.reduce((sum, record) => sum + Number(record.cost || 0), 0);
  const financeExpenseByCategory = hasCachedFinanceExpenses ? {} : (financeRecords || [])
    ...
    .reduce((groups, record) => {
      ...
      groups[category] = Number(groups[category] || 0) + Number(record.amount || 0);
      return groups;
    }, {});
  const financeExpenseTotal = Object.values(financeExpenseByCategory).reduce(
    (sum, amount) => sum + Number(amount || 0),
    0
  );
  const incomeSummary = summarizeFinanceIncome(financeRecords, milk.totalAmount);
  const totalOtherExpenses = Number(expenses.byCategory["इतर"] || 0) + aiExpense + Number(financeExpenseByCategory["इतर"] || 0);
  const totalMedicineExpenses = Number(expenses.byCategory["औषध"] || 0) + healthExpense;
  const totalAllExpenses = Number(expenses.monthlyTotal || 0) + healthExpense + aiExpense + financeExpenseTotal;
  ...
  const summary = {
    ...
    total_feed_expenses: Number(expenses.byCategory["चारा"] || 0) + Number(financeExpenseByCategory["चारा"] || 0),
    total_straw_expenses: expenses.byCategory["भूसा"] || 0,
    total_medicine_expenses: totalMedicineExpenses,
    total_labor_expenses: Number(expenses.byCategory["मजुरी"] || 0) + Number(financeExpenseByCategory["मजुरी"] || 0),
    total_transport_expenses: Number(expenses.byCategory["परिवहन"] || 0) + Number(financeExpenseByCategory["परिवहन"] || 0),
    ...
    net_profit: Number(incomeSummary.totalIncome || 0) - totalAllExpenses - deductionsCountedInProfit
  };
  ...
  return {
    data: {
      ...
      report: {
        ...
        healthExpenses: healthRecords,
        aiExpenses: aiRecords,
        financeRecords,
...
export async function saveCalvingRecord(calvingData) {
  ...
  const calvingPayload = {
    ...
    calf_photo_url: calvingData.calf_photo_url || null,
    calf_photo_storage_path: calvingData.calf_photo_storage_path || null,
  ...
  if (isOnline()) {
    ...
    if (calvingRecord?.completedLifecycleReminders?.length > 0) {
      await cacheReminders(calvingRecord.completedLifecycleReminders, farmId);
    }
    if (calvingRecord?.followupReminder) {
      await cacheReminders([calvingRecord.followupReminder], farmId);
    }
  ...
  if (calvingData.ai_record_id) {
    await markCachedRelatedRemindersDone(
      calvingData.ai_record_id,
      [
        PREGNANCY_CHECK_REMINDER_TYPE,
        MISSED_PREGNANCY_REMINDER_TYPE,
        DRY_OFF_REMINDER_TYPE,
  ...
  const [followupReminder] = calculateCalvingFollowupReminders(
    calvingData.actual_date,
    calvingData.cowName || calvingData.cow?.name || "गाय"
  );
  await addLocalReminder({
    ...followupReminder,
    farm_id: farmId,
