...
import { addDaysToISODate, calculateAIReminders } from "@/lib/reminderUtils";
...
export async function fetchJson(url, options = {}) {
  const { fresh = true, unwrapData = true, ...fetchOptions } = options;
...
export async function fetchCowProfile(cowId) {
  ...
  if (isOnline()) {
    try {
      ...
      await cacheMilkRecords(data?.records?.milk_records || [], farmId);
  ...
  const [aiRecords, healthRecords, milkRecords] = await Promise.all([
    getCachedAIByCow(cowId, farmId),
    getCachedHealthByCow(cowId, farmId),
    getCachedMilkByCow(cowId, 60, farmId)
  ]);
...
export async function saveMilkRecord(milkData) {
  ...
  const localId = await queueAction({
    ...
    cowName: milkData.cowName || milkData.cows?.name || ""
  });
...
export async function saveHealthRecord(healthData) {
  ...
  if (healthData.next_due_date) {
    await addLocalReminder({
      ...
      type: healthData.type === "जंतनाशक" ? "जंतनाशक" : healthData.type === "लसीकरण" ? "लसीकरण" : "तपासणी",
      message:
        healthData.type === "लसीकरण" || healthData.type === "जंतनाशक"
          ? `${healthData.cowName || "गाय"} ला ${healthData.vaccine_name || "लस"} देण्याची वेळ झाली`
          : `${healthData.cowName || "गाय"} ची पुढील तपासणी करा`,
...
export async function saveCalvingRecord(calvingData) {
  ...
  const calvingPayload = {
    ...
    calf_weight: calvingData.calf_weight ?? null,
    ...
    calf_identification_mark: calvingData.calf_identification_mark || null
  };
  ...
  if (isOnline()) {
    ...
    const cowData = await fetchJson(`/api/cows/${calvingData.cow_id}`, {
      method: "PUT",
      body: JSON.stringify({ status: "व्याललेली", farm_id: farmId })
    });
    await updateCachedCow(cowData, farmId);

    const reminderRecord = await fetchJson("/api/reminders", {
      method: "POST",
      body: JSON.stringify({
        ...reminderPayload,
    ...
    await cacheReminders([reminderRecord], farmId);

    if (calvingData.reminderId) {
      await fetchJson("/api/reminders", {
        method: "PATCH",
        body: JSON.stringify({ action: "done", id: calvingData.reminderId })
      });
    }
  ...
  await queueAction({
    type: "UPDATE",
    entity: "cow",
    endpoint: `/api/cows/${calvingData.cow_id}`,
    method: "PUT",
    payload: { status: "व्याललेली", farm_id: farmId },
    farm_id: farmId,
    cow_id: calvingData.cow_id,
    cowName: calvingData.cowName || ""
  });
