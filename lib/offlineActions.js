...
export async function fetchAccountingSummary(month, year) {
  ...
  const healthExpense = healthRecords.reduce((sum, record) => sum + Number(record.cost || 0), 0);
  const aiExpense = aiRecords.reduce((sum, record) => sum + Number(record.cost || 0), 0);
  const financeExpenseByCategory = (financeRecords || [])
    .filter((record) => record.type === "खर्च")
    .filter((record) => record.accounting_period !== "annual")
    .filter((record) => !isKhadyaExpenseCategory(record.category))
