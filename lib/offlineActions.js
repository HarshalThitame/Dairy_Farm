...
import { getMonthYearString, summarizeDairySlips, summarizeExpenses, summarizeSettlements } from "@/lib/accountingUtils";
...
export async function uploadSlipImage(imageBlob, metadata = {}) {
  ...
  metadata.onProgress?.({ stage: "compressing", message: "फोटो संकुचित करत आहे..." });
...
export async function extractSlipUpload(uploadId) {
  if (!isOnline()) {
...
export async function saveSettlement(settlementData, settlementId = null) {
  ...
  const localSettlement = {
    ...
    total_deductions: Number(payload.cattle_feed_deduction || 0) + Number(payload.other_deductions || 0),
    net_payable:
...
export async function fetchAccountingSummary(month, year) {
  ...
  const milk = slipsResult.data.monthlyTotal;
  ...
  const summary = {
    ...
    total_dairy_deductions: settlements.totalDeductions,
    net_profit: Number(milk.totalAmount || 0) - Number(expenses.monthlyTotal || 0) - Number(settlements.totalDeductions || 0)
  };
