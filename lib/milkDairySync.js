...
function buildSessionSlip(milkRecord, session, farmDefaults = {}) {
  ...
  return {
    ...
    slip_time: milkRecord.slip_time || null,
    session,
    milk_type: milkRecord.milk_type || "cow",
    dairy_name: farmDefaults.dairy_name || null,
    dairy_member_number: farmDefaults.dairy_member_number || null,
    dairy_member_code: cleanText(milkRecord.dairy_member_code) || farmDefaults.dairy_member_number || null,
    liters,
    ...
    ) ?? toOptionalNumber(milkRecord.clr_score) ?? toOptionalNumber(milkRecord.degree_reading),
    clr_score: toOptionalNumber(
      isMorning ? milkRecord.morning_degree_reading : milkRecord.evening_degree_reading
    ) ?? toOptionalNumber(milkRecord.clr_score) ?? toOptionalNumber(milkRecord.degree_reading),
    rate_per_liter: rate,
...
export async function recomputeMilkRecordFromDairySlips(supabase, farmId, date) {
  ...
  const primarySlip = morningSlip || eveningSlip;
  const payload = {
    farm_id: farmId,
    cow_id: null,
    date,
    slip_time: primarySlip?.slip_time || null,
    milk_type: primarySlip?.milk_type || eveningSlip?.milk_type || "cow",
    dairy_member_code: primarySlip?.dairy_member_code || primarySlip?.dairy_member_number || null,
    ...
    clr_score: toOptionalNumber(primarySlip?.clr_score ?? primarySlip?.clr_degree),
