...
export function summarizeSettlements(settlements = []) {
  return settlements.reduce(
    (summary, settlement) => ({
      ...
      anamatDeduction: roundMoney(summary.anamatDeduction + Number(settlement.anamat_cut || 0)),
...
export function getDeductionsCountedInProfit(settlementSummary = {}) {
  // खाद्य/इतर कपात हा प्रत्यक्ष खर्च आहे. अनामत खर्च नाही; ती वेगळ्या खात्यात जमा राहते.
...
export async function refreshMonthlySummary(supabase, farmId, month, year) {
  ...
  const payload = {
    ...
    total_anamat_accumulated: settlementSummary.anamatDeduction || 0,
...
export async function buildProfitTrend(supabase, farmId, month, year) {
  ...
  for (const item of months) {
    ...
    summaries.push({
      ...
      anamat: Number(summary.total_anamat_accumulated || 0)
    });
  }
