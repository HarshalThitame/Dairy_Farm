...
export function getDeductionsCountedInProfit(settlementSummary = {}) {
  // खाद्य कपात is a settlement/payment detail. खाद्य cost is counted from खर्च records.
  return roundMoney(settlementSummary.otherDeductions || 0);
}
...
export async function refreshMonthlySummary(supabase, farmId, month, year) {
  ...
  const slipSummary = summarizeDairySlips(slipsResult.data || []).monthlyTotal;
  ...
  const payload = {
    ...
    total_milk_income: slipSummary.totalAmount,
    total_liters: slipSummary.totalLiters,
