...
async function fetchSettlements(supabase, farmId, startDate, endDate) {
  const { data, error } = await supabase
    ...
    .select("id, period_start, period_end, settlement_date, total_liters, total_milk_income, cattle_feed_deduction, other_deductions, ai_raw_data")
    .eq("farm_id", farmId)
...
async function fetchExpenseInputs(supabase, farmId, startDate, endDate) {
  const [monthlyExpensesResult, financeRecordsResult, healthRecordsResult] = await Promise.all([
    supabase
...
async function summarizeExpenseRange(supabase, farmId, startDate, endDate) {
  ...
  return {
    ...
    expenseCount: combinedExpenses.filter((expense) => expense.info_only !== true).length,
...
export async function getProfit({ supabase, farmId, args }) {
  ...
  if (revenue.noData && expenses.totalExpense <= 0) {
    return noData(range);
  }
  ...
  return {
    ...
    netProfit: roundMoney(numberOrZero(revenue.revenue) - expenses.totalExpense)
  };
}
...
export async function getMonthlySummary({ supabase, farmId, args }) {
  const range = normalizeToolDateRange(args);
  const [milk, revenue, expenses] = await Promise.all([
  ...
  if (milk.totalMilk <= 0 && revenueValue <= 0 && expenses.totalExpense <= 0) {
    return noData(range);
  }
  ...
  return {
    ...
    directExpense: expenses.directExpense,
    dairyDeductions: expenses.dairyDeductions,
    netProfit: roundMoney(revenueValue - expenses.totalExpense)
  };
}
