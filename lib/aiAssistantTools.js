...
export async function getAnimalSummary({ supabase, farmId }) {
  const [cowsResult, calvesResult] = await Promise.all([
    supabase
      .from("cows")
    ...
    supabase
      .from("calves")
      .select("id, gender, status, is_raised")
      .eq("farm_id", farmId)
  ]);

  if (cowsResult.error) throw cowsResult.error;
  if (calvesResult.error) throw calvesResult.error;

  const cows = cowsResult.data || [];
  const calves = calvesResult.data || [];
  const activeCalves = calves.filter((calf) => calf.status === "active");

  if (!cows.length && !activeCalves.length) {
    return {
      noData: true,
      message: "या डेअरीसाठी जनावरांची माहिती उपलब्ध नाही."
    };
  }

  return {
    totalCows: cows.length,
    pregnantCows: cows.filter((cow) => cow.status === "गाभण").length,
    emptyCows: cows.filter((cow) => cow.status === "रिकामी").length,
    recentlyCalvedCows: cows.filter((cow) => cow.status === "व्याललेली").length,
    dryCows: cows.filter((cow) => cow.status === "वाळलेली").length,
    treatmentCows: cows.filter((cow) => cow.status === "उपचार सुरू").length,
    totalCalves: activeCalves.length,
    femaleCalves: activeCalves.filter((calf) => calf.gender === "मादी").length,
    maleCalves: activeCalves.filter((calf) => calf.gender === "नर").length,
    raisedCalves: activeCalves.filter((calf) => calf.is_raised !== false).length,
    soldCalves: calves.filter((calf) => calf.status === "sold").length,
    convertedCalves: calves.filter((calf) => calf.status === "converted_to_cow").length
  };
}
