...
import { getClientAuthToken, safeGetLocalStorageItem, safeSetLocalStorageItem } from "@/lib/clientStorage";
...
function getLastCacheRefreshTime() {
  return Number(safeGetLocalStorageItem(CACHE_REFRESH_KEY, "0") || 0);
}

function setLastCacheRefreshTime() {
  safeSetLocalStorageItem(CACHE_REFRESH_KEY, String(Date.now()));
}
...
async function updateLocalCache(entity, data) {
  ...
  if (entity === "cow") {
    ...
    await cacheFinanceRecords(Array.isArray(data) ? data : [data]);
    ...
    if (data.completedLifecycleReminders?.length > 0) {
      await cacheReminders(data.completedLifecycleReminders);
    }

    if (data.followupReminder) {
      await cacheReminders([data.followupReminder]);
    }
  }
}
...
export async function resolveConflict(localRecord, serverRecord) {
  ...
  if (localRecord.entity === "milk") {
    const payload = localRecord.payload || {};
    const preferPayloadValue = (field) =>
      payload[field] === undefined ? serverRecord[field] ?? null : payload[field];

    return {
      ...serverRecord,
      morning_litres: preferPayloadValue("morning_litres"),
      evening_litres: preferPayloadValue("evening_litres"),
      price_per_litre: preferPayloadValue("price_per_litre"),
      morning_price_per_litre: preferPayloadValue("morning_price_per_litre"),
      evening_price_per_litre: preferPayloadValue("evening_price_per_litre"),
      fat_percentage: preferPayloadValue("fat_percentage"),
      morning_fat_percentage: preferPayloadValue("morning_fat_percentage"),
      evening_fat_percentage: preferPayloadValue("evening_fat_percentage"),
      snf_value: preferPayloadValue("snf_value"),
      morning_snf_value: preferPayloadValue("morning_snf_value"),
      evening_snf_value: preferPayloadValue("evening_snf_value"),
      degree_reading: preferPayloadValue("degree_reading"),
      morning_degree_reading: preferPayloadValue("morning_degree_reading"),
      evening_degree_reading: preferPayloadValue("evening_degree_reading"),
      notes: preferPayloadValue("notes")
    };
  }
...
async function syncSingleRecord(record) {
  ...
  if ((record.entity === "health" || record.entity === "vaccination") && result.followupReminder) {
    await cacheReminders([result.followupReminder]);
  }
