...
-- Remove the discontinued "माज तपासणी" reminder feature everywhere at DB level:
-- 1. Delete existing rows for this removed reminder type.
-- 2. Recreate the reminder type constraint without this type.
-- 3. Replace the AI trigger so future AI records do not create 21-day heat reminders.

ALTER TABLE public.reminders
DROP CONSTRAINT IF EXISTS reminders_type_check;

DELETE FROM public.reminders
WHERE btrim(COALESCE(type, '')) IN (
  'माज तपासणी',
  'माज',
  'माज चेक',
  'heat_check',
...
SELECT COUNT(*) AS remaining_heat_check_reminders
FROM public.reminders
WHERE type = 'माज तपासणी';
